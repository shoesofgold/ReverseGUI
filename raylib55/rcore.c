...
void InitWindow(int width, int height, const char *title)
{
    ...
    CORE.Window.screenScale = rmMatrixIdentity();     // No draw scaling required by default
...
VrStereoConfig LoadVrStereoConfig(VrDeviceInfo device)
{
    ...
    if (rlGetVersion() != RL_OPENGL_11)
    {
       ...
       // float fovy = 2.0f*(float)atan2f(device.vScreenSize*0.5f, device.eyeToScreenDistance);
        ...
        config.projection[0] = rmMatrixMultiply(proj, MatrixTranslate(projOffset, 0.0f, 0.0f));
        config.projection[1] = rmMatrixMultiply(proj, MatrixTranslate(-projOffset, 0.0f, 0.0f));
...
Ray GetScreenToWorldRayEx(Vector2 position, Camera camera, int width, int height)
{
    ...
    Matrix matProj = rmMatrixIdentity();
...
Matrix GetCameraMatrix2D(Camera2D camera)
{
    ...
    matTransform = rmMatrixMultiply(rmMatrixMultiply(matOrigin, rmMatrixMultiply(matScale, matRotation)), matTranslation);
