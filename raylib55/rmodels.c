...
Model LoadModel(const char *fileName)
{
    ...
    model.transform = rmMatrixIdentity();
...
void DrawMesh(Mesh mesh, Material material, Matrix transform)
{
    ...
    Matrix matModel = rmMatrixIdentity();
    Matrix matView = rlGetMatrixModelview();
    Matrix matModelView = rmMatrixIdentity();
    ...
    matModel = rmMatrixMultiply(transform, rlGetMatrixTransform());
    ...
    matModelView = rmMatrixMultiply(matModel, matView);
    ...
    for (int eye = 0; eye < eyeCount; eye++)
    {
        ...
        Matrix matModelViewProjection = rmMatrixIdentity();
        if (eyeCount == 1) matModelViewProjection = rmMatrixMultiply(matModelView, matProjection);
        else
        {
            // Setup current eye viewport (half screen width)
            rlViewport(eye*rlGetFramebufferWidth()/2, 0, rlGetFramebufferWidth()/2, rlGetFramebufferHeight());
            matModelViewProjection = rmMatrixMultiply(rmMatrixMultiply(matModelView, rlGetMatrixViewOffsetStereo(eye)), rlGetMatrixProjectionStereo(eye));
        }
...
void DrawMeshInstanced(Mesh mesh, Material material, const Matrix *transforms, int instances)
{
    ...
    matModelView = rmMatrixMultiply(rlGetMatrixTransform(), matView);
    ...
    if ((anim.frameCount > 0) && (anim.bones != NULL) && (anim.framePoses != NULL))
    {
        ...
        if (firstMeshWithBones != -1)
        {
            ...
            for (int boneId = 0; boneId < anim.boneCount; boneId++)
            {
                ...
                Matrix bindMatrix = rmMatrixMultiply(rmMatrixMultiply(
                    MatrixScale(bindTransform->scale.x, bindTransform->scale.y, bindTransform->scale.z),
                    QuaternionToMatrix(bindTransform->rotation)),
                    MatrixTranslate(bindTransform->translation.x, bindTransform->translation.y, bindTransform->translation.z));

                Transform *targetTransform = &anim.framePoses[frame][boneId];
                Matrix targetMatrix = rmMatrixMultiply(rmMatrixMultiply(
                ...
                model.meshes[firstMeshWithBones].boneMatrices[boneId] = rmMatrixMultiply(MatrixInvert(bindMatrix), targetMatrix);
            }
...
void DrawModelEx(Model model, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale, Color tint)
{
    ...
    Matrix matTransform = rmMatrixMultiply(rmMatrixMultiply(matScale, matRotation), matTranslation);

    // Combine model transformation matrix (model.transform) with matrix generated by function parameters (matTransform)
    model.transform = rmMatrixMultiply(model.transform, matTransform);
...
static Model LoadIQM(const char *fileName)
{
    ...
    */
    ...
    for (int i = 0; i < model.meshCount; i++)
    {
        ...
        {
            model.meshes[i].boneMatrices[j] = rmMatrixIdentity();
        }
    }
    ...
    if (result == cgltf_result_success)
    {
        ...
        for (unsigned int i = 0; i < data->nodes_count; i++)
        {
            ...
            for (unsigned int p = 0; p < mesh->primitives_count; p++)
            {
                ...
                {
                    model.meshes[meshIndex].boneMatrices[j] = rmMatrixIdentity();
                }
